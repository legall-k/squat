...
qts_transpose_MatQ_new <- function(X) {
  X %>% 
...
qts_transconj_MatQ_new<-function(X){
  return(qts_transpose_MatQ_new(qts_conjugate_MatQ(X)))
}
...
qts_SVDH<-function(A, nb.axis = 2){
  ...
  return(list('U'=U,'V'=V, 'singular_values'=sing_values,'eigenvalues'=eigenvalues, 'inertia'=inertia, 'svdh_A'=svdh_A, 'nb.axis'=nb.axis, 'time'=(end.time-start.time)))
}
...
#' Tangent Quaternionic Principal Component Analysis
#'
#' @param X a list of qts
#' @param nb.axis a number
#' @param center a boolean
#' @description this function computes a TQPCA on an input data matrix, the user decides if data must be
#' centered 
#'
#' @return a list with :
#' F : coordinates of the inidividuals on the chosen components
#' G : coordinates of the variables (times) on the chosen components
#' U : the quaternionic matrix containing left eigenvectors
#' V : the quaternionic matrix containing right eigenvectors
#' eigenvalues : the list of eigenvalues
#' inertia : the inertia represented by the chosen components
...
#'
#' @examples
qts_tangent_QPCA_new <- function(X, nb.axis = 26, center = TRUE) {
  X <- scale_qts(X, center = center, standardize = FALSE, by_row = FALSE)
  N <- length(X)
  X <- purrr::map(X, log_qts)
  svdh <- qts_SVDH(X, nb.axis = nb.axis)
  eigenvalues <- svdh$eigenvalues / N
  U <- svdh$U
  V <- svdh$V
  V_trans <- V %>% 
    qts_transpose_MatQ_new() %>% 
    purrr::map(list) %>% 
    purrr::map(qts_transpose_MatQ_new) %>% 
    `[`(1:nb.axis)
  
  Fcomp <- V_trans %>% 
    purrr::map(~ qts_Hmatprod(X, .x)) %>% 
    purrr::map(qts_transpose_MatQ_new)
  
  Gcomp <- V_trans %>% 
    purrr::map2(eigenvalues[1:nb.axis], ~ qts_mult_MatQ(sqrt(.y), .x)) %>% 
    purrr::map(qts_transpose_MatQ_new)
